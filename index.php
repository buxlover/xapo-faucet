...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST["new_password"])) {
  ...
  if ($row === null || $row['claimed_at'] <= $time - ($settings['timer'] * 60)) {
    ...
    if($response->success){
      ...
      $qUrl=$sql->prepare("SELECT * FROM users WHERE username='?'");
      $qUrl.execute(array($username));
      $rowUrl=$qUrl->fetch();
      $url = get_main_url()."?r=". $rowUrl["id"];
      ...
      $q = $sql->prepare("Select * from referals where id=?");
      ...
      if(!$row){
        ...
        $qR=$sql->prepare("SELECT username FROM users WHERE id=?");
        $qR.execute(array($_GET["r"]));
        $rowR=$qR->fetch();
        $ref=$rowR["username"];
